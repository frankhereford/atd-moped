-e -- Most recent migration: moped-database/migrations/1707416196012_add_check_for_deleted_components/up.sql
...
CREATE VIEW project_list_view AS  WITH project_person_list_lookup AS (
 ...
 SELECT mp.project_id,
    ...
    ( SELECT max(phases.phase_end) AS max
           FROM moped_proj_phases phases
          WHERE true AND phases.project_id = mp.project_id AND phases.phase_id = 11 AND phases.is_deleted = false) AS completion_end_date,
    ( SELECT min(min_confirmed_dates.min_confirmed_date) AS min
           FROM ( SELECT min(phases.phase_start) AS min_confirmed_date
                   FROM moped_proj_phases phases
                ...
                UNION ALL
                 SELECT min(phases.phase_end) AS min_confirmed_date
                   FROM moped_proj_phases phases
                     LEFT JOIN moped_phases ON phases.phase_id = moped_phases.phase_id
                  WHERE true AND phases.phase_end IS NOT NULL AND phases.is_phase_end_confirmed = true AND phases.project_id = mp.project_id AND moped_phases.phase_name_simple = 'Complete'::text AND phases.is_deleted = false) min_confirmed_dates) AS substantial_completion_date,
